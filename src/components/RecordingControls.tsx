  ...
  useEffect(() => {
    const checkTauri = async () => {
      try {
        ...
        alert(t('recording.initializeFailed'));
      }
    };
    checkTauri();
  }, []);
  ...
  const handleStartRecording = useCallback(async () => {
    ...
    try {
      ...
      if (errorMsg.includes('microphone') || errorMsg.includes('mic') || errorMsg.includes('input')) {
        setDeviceError({
          title: t('recording.microphoneUnavailableTitle'),
          message: t('recording.microphoneUnavailableMessage')
        });
      } else if (errorMsg.includes('system audio') || errorMsg.includes('speaker') || errorMsg.includes('output')) {
        setDeviceError({
          title: t('recording.systemAudioUnavailableTitle'),
          message: t('recording.systemAudioUnavailableMessage')
        });
      } else if (errorMsg.includes('permission')) {
        setDeviceError({
          title: t('recording.permissionRequiredTitle'),
          message: t('recording.permissionRequiredMessage')
        });
      } else {
        setDeviceError({
          title: t('recording.failedTitle'),
          message: t('recording.failedMessage')
        });
      }
    }
        ...
        {deviceError && (
          <Alert variant="destructive" className="mt-4 border-red-300 bg-red-50">
            ...
            <button
              ...
              aria-label={t('recording.closeAlert')}
            >
              <X className="h-4 w-4" />
            </button>
