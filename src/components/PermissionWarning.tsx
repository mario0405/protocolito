        ...
        <Alert variant="destructive" className="border-amber-400 bg-amber-50">
          ...
          <AlertTitle className="text-amber-900 font-semibold">
            <div className="flex items-center gap-2">
              ...
              {!hasMicrophone && !hasSystemAudio ? t('permission.required') : !hasMicrophone ? t('permission.microphoneRequired') : t('permission.systemAudioRequired')}
            </div>
          </AlertTitle>
          {/* Action Buttons */}
          ...
          <AlertDescription className="text-amber-800 mt-2">
            ...
            {!hasMicrophone && (
              <>
                <p className="mb-3">
                  {t('permission.microphoneDescription')}
                </p>
                <div className="space-y-2 text-sm mb-4">
                  <p className="font-medium">{t('permission.pleaseCheck')}</p>
                  <ul className="list-disc list-inside ml-2 space-y-1">
                    <li>{t('permission.microphoneConnected')}</li>
                    <li>{t('permission.microphoneGranted')}</li>
                    <li>{t('permission.noExclusiveMic')}</li>
                  </ul>
                </div>
              </>
            )}
            ...
            {!hasSystemAudio && (
              <>
                <p className="mb-3">
                  {hasMicrophone
                    ? t('permission.systemAudioMissingWithMic')
                    : t('permission.systemAudioMissing')}
                </p>
                {isMacOS && (
                  <div className="space-y-2 text-sm mb-4">
                    <p className="font-medium">{t('permission.enableSystemAudioMac')}</p>
                    <ul className="list-disc list-inside ml-2 space-y-1">
                      <li>{t('permission.installVirtualAudio')}</li>
