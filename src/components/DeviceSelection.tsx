...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const fetchDevices = async () => {
    try {
      ...
      setError(t('recording.devicePreferencesSaveFailed'));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const startAudioLevelMonitoring = async () => {
    try {
      ...
      if (deviceNames.length === 0) {
        setError(t('recording.noMicrophones'));
        return;
      }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-900">{t('recording.audioDevices')}</h4>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="default">{t('recording.defaultMicrophone')}</SelectItem>
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('recording.noMicrophones')}</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600 font-medium">{t('recording.microphoneLevels')}</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="system-selection" className="w-full">
              <SelectValue placeholder={t('recording.selectSystemAudio')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">{t('recording.defaultSystemAudio')}</SelectItem>
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('recording.noSystemAudio')}</p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• <strong>{t('recording.microphone')}:</strong> {t('recording.microphoneHelp')}</p>
        <p>• <strong>{t('recording.systemAudio')}:</strong> {t('recording.systemAudioHelp')}</p>
        {isMonitoring && (
          <p>• <strong>{t('recording.microphoneLevels')}</strong> {t('recording.micLevelsHelp')}</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• <strong>{t('common.note')}</strong> {t('recording.testMicTip')}</p>
        )}
      </div>
    </div>
  );
