...
export function useCopyOperations({
  ...
  const handleSendSummaryToIntegrations = useCallback(async () => {
    try {
      let summaryMarkdown = '';
      ...
      if (!summaryMarkdown.trim()) {
        summaryMarkdown = summaryToMarkdown(aiSummary);
      }

      if (!summaryMarkdown.trim()) {
        toast.error('No summary content available to send');
        return;
      }
      ...
      const results = await sendSummaryToIntegrations(payload);
      const sent = results.filter((result) => result.status === 'sent');
      const copied = results.filter((result) => result.status === 'copied');
      const failed = results.filter((result) => result.status === 'error');

      if (sent.length > 0) {
        toast.success(`Sent to ${sent.map((result) => INTEGRATION_LABELS[result.provider]).join(', ')}`);
      } else if (copied.length > 0) {
        toast.success('Integration package copied');
      }

      if (failed.length > 0) {
        toast.error(`Failed: ${failed.map((result) => INTEGRATION_LABELS[result.provider]).join(', ')}`, {
          description: failed[0].message,
        });
      }

      await Analytics.track('summary_sent_to_integrations', {
        meeting_id: meeting.id,
        sent_count: sent.length.toString(),
        copied_count: copied.length.toString(),
        failed_count: failed.length.toString(),
      });
    } catch (error) {
      console.error('Failed to send summary to integrations:', error);
      toast.error('Failed to send summary');
    }
  }, [aiSummary, blockNoteSummaryRef, meeting, meetingTitle]);

  return {
    handleCopyTranscript,
    handleCopySummary,
    handleSendSummaryToIntegrations,
  };
}
