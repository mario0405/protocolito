...
export function useSummaryGeneration({
  ...
  const sendAutoIntegrations = useCallback(async (summary: Summary | { markdown?: string }, meetingName?: string) => {
    const autoSendConfigs = loadIntegrations().filter((config) => config.enabled && config.autoSendSummary);
    if (autoSendConfigs.length === 0) return;

    const summaryMarkdown = 'markdown' in summary && typeof summary.markdown === 'string'
      ? summary.markdown
      : summaryToMarkdown(summary as Summary);

    if (!summaryMarkdown.trim()) return;

    try {
      const results = await sendSummaryToIntegrations(
        buildIntegrationPayload({
          meetingId: meeting.id,
          title: meetingName || meeting.title || meeting.name || 'Meeting',
      ...
      if (sentCount > 0) {
        toast.success('Protocol sent to integrations', {
          description: `${sentCount} target${sentCount === 1 ? '' : 's'} updated`,
        });
      } else if (copiedCount > 0) {
        toast.info('Protocol copied for integration import');
      }

      if (errorCount > 0) {
        toast.error('Some integrations failed', {
          description: `${errorCount} target${errorCount === 1 ? '' : 's'} could not be updated`,
        });
      }

      await Analytics.track('summary_auto_sent_to_integrations', {
        sent_count: sentCount.toString(),
        copied_count: copiedCount.toString(),
        error_count: errorCount.toString(),
      });
    } catch (error) {
      console.error('Failed to auto-send summary to integrations:', error);
      toast.error('Failed to auto-send protocol');
    }
  }, [meeting.id, meeting.title, meeting.name, meeting.created_at]);
      ...
      startSummaryPolling(meeting.id, process_id, async (pollingResult) => {
        ...
        if (pollingResult.status === 'error' || pollingResult.status === 'failed') {
          ...
          toast.error(`Failed to ${isRegeneration ? 'regenerate' : 'generate'} summary`, {
            description: errorMessage.includes('Connection refused')
              ? 'Could not connect to the summary service. Please ensure Ollama is running or Infomaniak is configured.'
              : errorMessage,
          });
        ...
        if (pollingResult.status === 'completed' && pollingResult.data) {
          ...
          if (pollingResult.data.markdown) {
            ...
            await sendAutoIntegrations(markdownSummary, meetingName);
            return;
          }
          ...
          await sendAutoIntegrations(formattedSummary, meetingName);
    ...
    if (modelConfig.provider === 'ollama') {
      try {
        ...
        if (!models || models.length === 0) {
          toast.error(
            'No Ollama models found. Please download a model from Model Settings.',
            { duration: 5000 }
          );
          return;
        }
